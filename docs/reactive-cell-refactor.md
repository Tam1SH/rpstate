...
### 6. `*_external`: не баг, а семантика — задокументировать

Изначально стояло как «фильтр смотрит только на `Update`, остальное едет мимо — починить».
Оказалось наоборот: это замысел, и он был зафиксирован ассертом
`"Creation (Insert) is NOT ignored"` в `test_map_subscribe_external`.

Правило: **фильтруется только `Update`**. Правка значения — дело того, кто её сделал;
`Insert`/`Remove`/`Clear` меняют *состав* мапы и приезжают всем, потому что вью со списком
ключей обязан перестроиться независимо от автора.

- [x] Задокументировать на обоих `*_external`, в sync- и async-мапе.
- [x] Тесты на полную матрицу (существующий покрывал только `Insert`+`Update`).

**Острый край, который стоит помнить:** `set_or_create` возвращается тебе или нет в
зависимости от того, был ли ключ, — первый раз это `Insert`, дальше `Update`. Записано в доке.
...
`remove` стоит `false`, и они полагаются только на подписку стора. `clear` же делает и то, и
другое: стор удаляет ключи по одному, подписка честно превращает каждое удаление в `Remove`,
а сверху идёт прямой `core.notify(Clear)`. На мапе из двух ключей подписчик видит три события.

`true` там стоит не по недосмотру: у `MapChange::Clear` нет ключа, а подписка стора работает
по префиксу и пересобирает события **по ключам** — собрать из них `Clear` она не может в
принципе. Прямой notify закрывает архитектурный зазор, а не является лишней строкой.

**Измерено пробой, и это хуже, чем «дубль»:**

| подписчик | видит на `clear()` мапы из двух ключей |
|---|---|
| хендл, вызвавший clear (и его клоны/форки — у них общий `core`) | `Remove`, `Remove`, **`Clear`** |
| независимый экземпляр на том же сторе | `Remove`, `Remove` |

То есть `Clear` — не событие системы, а локальное эхо на том ядре, которое его вызвало.
Любой другой подписчик отличить очистку от поштучных удалений **не может в принципе**.

Варианты:

- **(а)** Флаг на `ReactiveMapCore`, подписка глушит свои удаления на время clear. Дёшево, но
  состояние и гонка — и чинит только половину: независимый экземпляр по-прежнему без `Clear`.
- **(б)** Убрать прямой notify. Консистентно с остальными операциями, но `MapChange::Clear`
  становится мёртвым вариантом enum'а.
- **(в)** Дать бэкенду `delete_prefix`: одно удаление → одно событие → подписка делает `Clear`.
  Единственный вариант, после которого `Clear` виден всем. Цена — новый `StoreOp` и правки в
  redb, sqlite и text.

**Разведка по объёму (в):** `StoreOp` разбирается ровно в одном месте вне бэкендов — подписка
мапы ([primitives_factory.rs:181,202](../crates/main/amethystate/src/store/primitives_factory.rs));
...
```
1 (ошибки) ──┬─> 3 (ReactiveCell) ──> 4 (entry store-first) ──> 6.5 (clear) ──> 8 (guinea) ──> 5 (Signal internal)
2 (Signal)  ─┘
6 (*_external) — сделано, независимо
7 (&T) — последним
```

6.5 идёт **до** guinea: он меняет то, что видит подписчик мапы, и мигрировать guinea на
поведение, которое сразу после этого поменяется, смысла нет.
