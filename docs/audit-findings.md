...
это список к разбору. Пометки: **[проверено]** — воспроизвёл сам, **[не проверял]** — со слов
аудита, выглядит правдоподобно, но нужен свой прогон.
...
### 1. Дебаунсер redb/sqlite теряет запись **[проверено]**
...
### 2. `flush_prefix` теряет буфер при ошибке записи **[не проверял]**
...
### 3. Гонка `has_pending` в text-бэкенде откатывает свежую запись **[не проверял]**
...
### 4. `ReactiveMapCore::notify` держит мьютексы во время колбэков **[проверено, дедлок воспроизведён]**
...
### 5. Field игнорирует `Delete` — кэш навсегда расходится с хранилищем **[не проверял]**
...
### 9. Превышение глубины интерцепторов пропускает изменение мимо валидации **[не проверял]**
...
### 10. `old` в событии `Set` у redb/sqlite берётся только из буфера **[не проверял]**
...
### 11. `remove` определяет существование ключа по кэшу **[не проверял]**

[map_ops.rs:144-147](../crates/core/amethystate-core/src/primitives/map_ops.rs)

Кэш заполняется только `initial_values` и последующими изменениями. Ключ, записанный предыдущим
...
### 12. Для `Clear` прогоняются key-интерцепторы всех ключей **[не проверял]**
