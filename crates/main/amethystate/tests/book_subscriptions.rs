...
#[test]
fn a_subscription_lasts_as_long_as_its_handle() -> Result<(), Box<dyn Error + Send + Sync>> {
    let (state, _path) = open("book_subs_raii")?;
...
#[test]
fn a_scope_holds_several_at_once() -> Result<(), Box<dyn Error + Send + Sync>> {
    let (state, _path) = open("book_subs_scope")?;
...
#[test]
fn ignoring_your_own_writes() -> Result<(), Box<dyn Error + Send + Sync>> {
    let (state, _path) = open("book_subs_external")?;
    let heard = Arc::new(Mutex::new(Vec::new()));
    let seen = Arc::clone(&heard);
...
#[test]
fn a_clone_is_the_same_actor_and_a_fork_is_not() -> Result<(), Box<dyn Error + Send + Sync>> {
    let (state, _path) = open("book_subs_identity")?;
...
#[test]
fn who_made_the_change() -> Result<(), Box<dyn Error + Send + Sync>> {
    let (state, _path) = open("book_subs_source")?;
    let heard = Arc::new(Mutex::new(Vec::new()));
    let seen = Arc::clone(&heard);
